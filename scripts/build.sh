...
if ! [[ $REMOTE = *"https://github.com/Authenticator-Extension/Authenticator.git"* || $REMOTE = *"git@github.com:Authenticator-Extension/Authenticator.git"* || $CI ]] ; then
    ...
    echo
    echo -e "This fork requires its own third-party API credentials before redistribution. Configure ./src/models/credentials.ts and the relevant manifest values."
    echo
fi
